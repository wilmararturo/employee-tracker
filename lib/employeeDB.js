...
class employeeDB {
    ...
    getAllEmployeeData(cb) {
        let query = `SELECT e.id,e.first_name,e.last_name,role.title,role.salary,department.name, `
        query += `(SELECT CONCAT(emp.first_name, " ", emp.last_name) FROM employee emp WHERE emp.id = e.manager_id LIMIT 1) as Manager `
